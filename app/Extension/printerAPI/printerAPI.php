<?php
  ...
  //用户
  //UKEY： krdJxMpnKnzg6z2w
  //USER： 454089127@qq.com
  
  header("Content-type: text/html; charset=utf-8");
  include 'HttpClient.class.php';

  define('USER', '454089127@qq.com');  //*必填*：飞鹅云后台注册账号
  define('UKEY', 'krdJxMpnKnzg6z2w');  //*必填*: 飞鹅云后台注册账号后生成的UKEY 【备注：这不是填打印机的KEY】
  define('SN', '921612618');      //*必填*：打印机编号，必须要在管理后台里添加打印机或调用API接口添加之后，才能调用API
  ...
  //标签说明：
  //单标签:
    //"<BR>"为换行,"<CUT>"为切刀指令(主动切纸,仅限切刀打印机使用才有果)
  ...
  //打印排版格式
  //名称$A 单价$B 数量$C 金额$D-->这里的字节数可按自己需求自由改写，14+6+3+6再加上代码写的3个空格就是32了，58mm打印机一行总占32字节
  function composing($arr,$A,$B,$C,$D,$storeName,$tableNum)
  {
    $orderInfo = '<CB>'.$storeName.'</CB><BR>';
    $orderInfo .= '<CB>'.$tableNum.'号桌</CB><BR>';
